...
def get_uv_coords(array_in, station1, station2, ra, dec, ha):
    """
    get the uv coordinates for the VLTI for a given ra, dec and
    hour angle
    ...
    v = B[1] * math.sin(dec) * math.sin(ha) + B[0] * (math.sin(latitude) * \
        math.sin(dec) * math.cos(ha) + math.cos(latitude) * math.cos(dec))
