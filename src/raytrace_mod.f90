...
CONTAINS
FUNCTION raytrace_px_generic(basics, grid, model, dust, gas,          &
    ...
    IMPLICIT NONE
        ...
        IF ( ray_len .gt. 0.0_r2 ) THEN
            ...
            DO WHILE ( dz_sum*(1.0_r2+epsr2*1.0e3) .lt. 2.0_r2* sqrt(ray_len) )
                ...
                ! now we need to calculate the intensity depending on the mode
                ! mode (continuum, lines)
                SELECT CASE(mode)
                CASE('LINE')
        ...
        END IF
    END FUNCTION raytrace_px_generic

    FUNCTION line_intensity(grid, model, dust, gas,                          &
                            coor_map1, coor_map2, i_map)                     &
                            RESULT(intensity)
    !----------------------------------------------------------------------!
        TYPE(Grid_TYP),INTENT(IN)                        :: grid
        ...
        IF ( (grid%grd_mol_density(nr_cell) .gt. 1.0e-200_r2) ) THEN
